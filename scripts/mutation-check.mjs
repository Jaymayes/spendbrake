...
const MUTANTS = [
  // ── budget gate ──────────────────────────────────────────────────────────
  // With a tolerance, a bare >= → > flip is an EQUIVALENT mutant (the tolerance subsumes the
  // boundary), so B1 removes both and B6 removes only the tolerance.
  { id: 'B1', file: 'src/budget-gate.ts', what: 'blocks only strictly ABOVE the cap',
    find: 'if (spentUsd >= capUsd - EPSILON_USD) return { allowed: false, reason: "cap_exceeded", spentUsd, capUsd };',
    replace: 'if (spentUsd > capUsd) return { allowed: false, reason: "cap_exceeded", spentUsd, capUsd };' },
  { id: 'B6', file: 'src/budget-gate.ts', what: 'cap tolerance removed (DEFECT-7 regresses)',
    find: 'if (spentUsd >= capUsd - EPSILON_USD) return { allowed: false, reason: "cap_exceeded", spentUsd, capUsd };',
    replace: 'if (spentUsd >= capUsd) return { allowed: false, reason: "cap_exceeded", spentUsd, capUsd };' },
  { id: 'B7', file: 'src/budget-gate.ts', what: 'unreadable tokens priced at $0 (DEFECT-10 regresses)',
    find: '    return Number.POSITIVE_INFINITY;\n  }\n  if (tokens === 0) return 0;',
    replace: '    return 0;\n  }\n  if (tokens === 0) return 0;' },
  { id: 'B8', file: 'src/budget-gate.ts', what: 'shared float tolerance set to zero',
    find: 'export const EPSILON_USD = 1e-9;', replace: 'export const EPSILON_USD = 0;' },
  ...
  { id: 'B5', file: 'src/budget-gate.ts', what: 'invalid cap falls back to UNLIMITED',
    find: 'return Number.isFinite(n) && n > 0 ? n : DEFAULT_HARD_CAP_USD;',
    replace: 'return Number.isFinite(n) && n > 0 ? n : Infinity;' },
  { id: 'B9', file: 'src/budget-gate.ts', what: 'finite check removed — an Infinity cap is accepted (DEFECT-8 regresses)',
    find: 'return Number.isFinite(n) && n > 0 ? n : DEFAULT_HARD_CAP_USD;',
    replace: 'return n > 0 ? n : DEFAULT_HARD_CAP_USD;' },
  { id: 'B10', file: 'src/budget-gate.ts', what: 'unreadable spend check removed (DEFECT-9 regresses)',
    find: '  if (Number.isNaN(spentUsd)) return { allowed: false, reason: "invalid_spend", spentUsd, capUsd };\n',
    replace: '' },
  { id: 'B11', file: 'src/budget-gate.ts', what: 'empty model priced as free again (DEFECT-11 regresses)',
    find: '  if (ZERO_COST.test(m)) return 0;\n  if (typeof tokens !== "number"',
    replace: '  if (!m || ZERO_COST.test(m)) return 0;\n  if (typeof tokens !== "number"' },
  { id: 'B12', file: 'src/budget-gate.ts', what: 'numeric-string readings rejected as corrupt (over-strict)',
    find: '  if (typeof x === "string" && x.trim() !== "") return Number(x);\n', replace: '' },
  ...
  { id: 'A2', file: 'src/approval-gate.ts', what: 'whitespace-only releaser accepted',
    find: 'typeof raw === "string" ? raw.trim() :', replace: 'typeof raw === "string" ? raw :' },
  { id: 'A6', file: 'src/approval-gate.ts', what: 'any truthy releaser (a boolean) counts as a release',
    find: 'typeof raw === "number" && Number.isFinite(raw) ? String(raw) : ""',
    replace: 'raw != null && raw !== "" ? String(raw) : ""' },
  { id: 'A7', file: 'src/approval-gate.ts', what: 'NaN accepted as a numeric releaser',
    find: 'typeof raw === "number" && Number.isFinite(raw) ? String(raw)', replace: 'typeof raw === "number" ? String(raw)' },
  ...
  { id: 'A4', file: 'src/approval-gate.ts', what: 'rejected item publishable on re-release',
    ...
    replace: '' },
  { id: 'A5', file: 'src/approval-gate.ts', what: 'allow-list removed — unknown statuses publish (DEFECT-5 regresses)',
    find: '  if (s.status !== "pending") return { allowed: false, reason: "unknown_status" };\n',
    replace: '' },
  ...
  { id: 'D1', file: 'src/disclosure.ts', what: '#ad loses its word-boundary prefix (matches "word#ad")',
    find: String.raw`/(^|[\s(>#])#ad(?![\w‐-―-])/i,`, replace: String.raw`/#ad(?![\w‐-―-])/i,` },
  { id: 'D18', file: 'src/disclosure.ts', what: '#ad hyphen lookahead reverted to \\b (DEFECT-2 regresses)',
    find: String.raw`/(^|[\s(>#])#ad(?![\w‐-―-])/i,`, replace: String.raw`/(^|[\s(>#])#ad\b/i,` },
  { id: 'D19', file: 'src/disclosure.ts', what: '#ai hyphen lookahead reverted to \\b',
    find: String.raw`/(^|[\s(>#])#ai(?![\w‐-―-])/i,`, replace: String.raw`/(^|[\s(>#])#ai\b/i,` },
  { id: 'D17', file: 'src/disclosure.ts', what: 'HTML-comment stripping removed (DEFECT-3 regresses)',
    find: String.raw`const visible = text.replace(/<!--[\s\S]*?(?:-->|$)/g, " ");`, replace: 'const visible = text;' },
  { id: 'D20', file: 'src/disclosure.ts', what: 'an UNTERMINATED comment no longer hides the rest',
    find: String.raw`/<!--[\s\S]*?(?:-->|$)/g`, replace: String.raw`/<!--[\s\S]*?-->/g` },
  ...
  { id: 'D5', file: 'src/disclosure.ts', what: '#aigenerated alternative deleted',
    find: String.raw`/(^|[\s(>#])#aigenerated(?![\w‐-―-])/i,`, replace: '' },
  { id: 'D6', file: 'src/disclosure.ts', what: '#ai alternative deleted',
    find: String.raw`/(^|[\s(>#])#ai(?![\w‐-―-])/i,`, replace: '' },
  ...
  { id: 'D9', file: 'src/disclosure.ts', what: 'whitespace-only content treated as content',
    find: 'if (!visible.trim()) {', replace: 'if (!visible) {' },
  { id: 'D10', file: 'src/disclosure.ts', what: 'required literals become case-sensitive',
    find: 'if (!visible.toLowerCase().includes(needle.toLowerCase())) missing.push(needle);',
    replace: 'if (!visible.includes(needle)) missing.push(needle);' },
  ...
  { id: 'D12', file: 'src/disclosure.ts', what: 'negation check disabled (DEFECT-1 regresses)',
    find: 'if (!isNegated(text, markerStart)) return true;', replace: 'return true;' },
  { id: 'D13', file: 'src/disclosure.ts', what: 'negation reach cut to one word',
    find: 'const NEGATION_REACH_WORDS = 3;', replace: 'const NEGATION_REACH_WORDS = 1;' },
  { id: 'D14', file: 'src/disclosure.ts', what: 'clause scope removed — negations reach across sentences',
    find: String.raw`const clause = text.slice(0, markerStart).split(/[.!?;:,()\n]/).pop() ?? "";`,
    replace: 'const clause = text.slice(0, markerStart);' },
  { id: 'D15', file: 'src/disclosure.ts', what: "curly apostrophe dropped (isn’t no longer a negation)",
    find: "/n['’]t$/.test(w)", replace: "/n't$/.test(w)" },
  { id: 'D16', file: 'src/disclosure.ts', what: '"non-" prefix no longer stripped to "non"',
    find: '.map((w) => w.replace(/^-+|-+$/g, ""))', replace: '.map((w) => w)' },
  ...
  { id: 'R4', file: 'src/reservation-gate.ts', what: 'admission tolerance removed',
    find: 'if (spentUsd + reservedUsd + est - capUsd > EPSILON_USD) {', replace: 'if (spentUsd + reservedUsd + est - capUsd > 0) {' },
  ...
  { id: 'R6', file: 'src/reservation-gate.ts', what: 'spend-at-cap check removed',
    find: 'if (spentUsd >= capUsd - EPSILON_USD) return { allowed: false, reason: "cap_exceeded", reserveUsd: 0, ...base };',
    replace: 'if (false) return { allowed: false, reason: "cap_exceeded", reserveUsd: 0, ...base };' },
  { id: 'R8', file: 'src/reservation-gate.ts', what: 'spend-at-cap tolerance removed',
    find: 'if (spentUsd >= capUsd - EPSILON_USD) return { allowed: false, reason: "cap_exceeded", reserveUsd: 0, ...base };',
    replace: 'if (spentUsd >= capUsd) return { allowed: false, reason: "cap_exceeded", reserveUsd: 0, ...base };' },
  { id: 'R9', file: 'src/reservation-gate.ts', what: 'unreadable spend/holds check removed (DEFECT-9 regresses)',
    find: '  if (unreadable) return { allowed: false, reason: "invalid_state", reserveUsd: 0, ...base };\n', replace: '' },
  ...
  { id: 'S2', file: 'src/reservation-sql.ts', what: 'reserve ignores spend already at the cap',
    find: 'AND w.total_spend_usd < w.hard_cap_usd - 1e-9', replace: 'AND 1 = 1' },
  ...
  { id: 'S5', file: 'src/reservation-sql.ts', what: 'settlement trips the switch only strictly ABOVE the cap',
    find: 'total_spend_usd + ?2 >= hard_cap_usd - 1e-9', replace: 'total_spend_usd + ?2 > hard_cap_usd' },
  { id: 'S8', file: 'src/reservation-sql.ts', what: 'settlement trip tolerance removed (DEFECT-7 regresses)',
    find: 'total_spend_usd + ?2 >= hard_cap_usd - 1e-9', replace: 'total_spend_usd + ?2 >= hard_cap_usd' },
  { id: 'S9', file: 'src/reservation-sql.ts', what: 'expiry trip tolerance removed',
    find: '${EXPIRED_FOR_ROW} >= hard_cap_usd - 1e-9', replace: '${EXPIRED_FOR_ROW} >= hard_cap_usd' },
  { id: 'S10', file: 'src/reservation-sql.ts', what: 'reserve spend-at-cap tolerance removed',
    find: 'AND w.total_spend_usd < w.hard_cap_usd - 1e-9', replace: 'AND w.total_spend_usd < w.hard_cap_usd' },
